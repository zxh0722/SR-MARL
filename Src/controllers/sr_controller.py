...
import numpy as np
import copy
from clusters.structural_entropy import *
...
class SRMAC:
    ...
    def update_role_action_spaces(self):
        ...
        labels, n_clusters, reprs = se_cluster(self.n_actions, action_repr_array)
        spaces = []
        # spaces.append(np.array([1.] * self.n_actions).astype(np.float64))
        for cluster_i in range(self.n_clusters):
        #     spaces.append((k_means.labels_ == cluster_i).astype(np.float))
            spaces.append((labels == cluster_i).astype(np.float))
        #
        ...
        self.role_latent = th.Tensor(np.array(reprs)).to(self.args.device).float()
