...
def se_cluster(n_actions, action_repr):
    ...
    # one_dimension_tree = algorithm.one_d()
    ...
    for children in three_dimension_tree.get_root().get_children():
        reprs.append(aggregate(children, action_repr, graph))
    # Step 4: 聚类结果
    labels, n_clusters = decode_two_dimension_tree(three_dimension_tree.get_root(), n_actions)
